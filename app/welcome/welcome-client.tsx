...
export default function WelcomeClient({
  ...
  ticker: { users: number; hours: number; sessions: number };
}) {
  ...
  const targetUsers = ticker.users;
  const targetHours = ticker.hours;
  const targetSessions = ticker.sessions;

  const userLive = useLiveCounter(mounted ? targetUsers : 0, 3000, mounted);
  const hoursLive = useLiveCounter(mounted ? targetHours : 0, 3500, mounted);
  const sessionsLive = useLiveCounter(mounted ? targetSessions : 0, 4000, mounted);
