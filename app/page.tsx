...
export default function Home() {
  ...
  const noneBackgroundGradient = useTimerStore(
    (state) => state.noneBackgroundGradient,
  );
  ...
  const activeTaskIsGroupTask = useTimerStore(
    (state) => state.activeTaskIsGroupTask,
  );
  const activeTaskSourceGroupId = useTimerStore(
    (state) => state.activeTaskSourceGroupId,
  );
  ...
  const hideTopLeftTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const priorityStyles: Record<
    string,
    { border: string; glow: string; text: string; dot: string; label: string }
  > = {
    urgent: {
      label: "Urgent",
  ...
  const p =
    priorityStyles[activeTaskPriority ?? "natural"] ?? priorityStyles.natural;

  return (
    <BackgroundTheme showSettings={false} isHomePage={true}>
      <div
        className="flex flex-col flex-1 bg-zinc-950 min-h-screen relative overflow-hidden font-sans"
        style={
          { "--font-sans": "var(--font-space-grotesk)" } as React.CSSProperties
        }
      >
          ...
          {activeTaskLabel && (
            <div className="fixed top-24 right-10 z-40 animate-in slide-in-from-right-12 fade-in duration-1000 ease-[cubic-bezier(0.23,1,0.32,1)]">
              <div className="relative group/card animate-float">
                ...
                <div
                  onClick={() =>
                    activeTaskNotes && setNotesExpanded(!notesExpanded)
                  }
                  className={cn(
                  ...
                  <div className="flex items-center justify-between w-full mb-5">
                    <div className="flex items-center gap-2.5">
                      <div className="relative flex items-center justify-center">
                        <div
                          className={cn(
                            "w-2 h-2 rounded-full animate-ping absolute opacity-70",
                            p.dot,
                          )}
                        />
                        <div
                      ...
                      <span
                        className={cn(
                          "text-[9px] font-black uppercase tracking-[0.25em] font-sans",
                          p.text,
                        )}
                      >
                        {p.label}
